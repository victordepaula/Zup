...
class MoviesApplication : NSObject {
    ...
    func  movies(onSuccess: @escaping (_ result: [Movie]) -> Void, onFailureMessage: @escaping (_ failureMessage: Error)-> Void, onFailure: @escaping (_ failure:Error)-> Void, stringForSearch: String){
    ...
    func   movies1(onSuccess: @escaping (_ result: [MoviesResponse]) -> Void, onFailureMessage: @escaping (_ failureMessage: Error)-> Void, onFailure: @escaping (_ failure:Error)-> Void,stringForSearch: String){
        guard let url = URL(string: getUrlMovieByName(personId: stringForSearch)) else {
            return
        }
        MoviesProxy().getMovies(url: url, onCompletion: {(response: String) -> Void in
