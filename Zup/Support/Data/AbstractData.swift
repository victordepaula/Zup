...
class AbstractData: NSObject {
    ...
    func getWithUserIdAndCountryId(value: String?, countryID: Int32?, attribute: Attributtes?) -> Any {
        let managedContext = getContext()
        let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: entityName)
        if let value1 = value, let atribute1 = attribute {
            fetchRequest.predicate = NSPredicate(format: "\(atribute1.rawValue) == %@", value1)
        }
        do {
    ...
    //    func updateCountryWithUserID(value: String?, country:Movie, attribute: Attributtes?) -> Bool{
    //        let managedContext = getContext()
    //        let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: entityName)
    ...
    //        do {
    //            let results = try managedContext.fetch(fetchRequest)
    //            let countryResult = results[0] as! Movie
